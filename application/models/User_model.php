...
class User_model extends CI_Model
{
    ...
    // check user login
    public function check_user($pcode,$card_id){
        $this->db->where('hrcode', $pcode);
        $this->db->where('card_id', $card_id);        
    ...
    public function hr_exists($userid){
   ...
   /*public function update_duration_data($data)
    {
        ...
        $this->db->where('created_at', $data['cdate']);
        unset($data['id']);
        unset($data['cdate']);
        $this->db->update('online_time', $data);
        //$this->db->trans_complete();
    ...
    /*public function user_exists($userid, $adate){
        $this->db->where('userid', $userid);
        $this->db->where('created_at', $adate);
        $query = $this->db->get('online_time');
        if($query->num_rows() > 0 ){ 
            return TRUE; 
        } else { 
            return FALSE; 
